...
namespace CashierRegister.Sql2Sqlite
{
    ...
    public class SqlServerToSQLite
    {
        ...
        public static void ConvertSqlServerToSQLiteDatabase(string sqlServerConnString,
            string sqlitePath, string password, bool createTriggers, bool createViews)
        {
            ...
            WaitCallback wc = new WaitCallback(delegate(object state)
            {
                ...
                {
                    _isActive = true;
                    ConvertSqlServerDatabaseToSQLiteFile(sqlServerConnString, sqlitePath, password, createTriggers, createViews);
                    _isActive = false;
                    _strMessage = "Success";
                }
                catch (Exception ex)
                {
                    ...
                    _isActive = false;
                    _strMessage = "Failed to convert SQL Server database to SQLite database";
                } // catch
            });
            ThreadPool.QueueUserWorkItem(wc);
            return _strMessage;*/
            //return "Failed to convert SQL Server database to SQLite database";
            try
            {
                ConvertSqlServerDatabaseToSQLiteFile(sqlServerConnString, sqlitePath, password, createTriggers, createViews);
                GC.Collect();
            ...
            {
                //return "Failed to convert SQL Server database to SQLite database "+ex.Message;
        ...
        /*NVTAM*/
        public static string ConvertSqlServerToSQLDatabase(string sqlServerConnString,
            string sqlitePath, string password, bool createTriggers, bool createViews)
        {
            _cancelled = false;
            try
            {
                ConvertSqlServerDatabaseToSQLFile(sqlServerConnString, sqlitePath, password, createTriggers, createViews);
                return "Success";
            }
            catch (Exception ex)
            {
                return "Failed to convert SQL Server database to SQL database";
            }
        }
        private static void ConvertSqlServerDatabaseToSQLFile(
            string sqlConnString, string sqlitePath, string password, bool createTriggers, bool createViews)
        {
            ...
            CreateSQLDatabase(sqlConnString, sqlitePath, ds, password, createViews);
            // Add triggers based on foreign key constraints
            if (createTriggers)
                AddTriggersForForeignKeys(sqlitePath, ds.Tables, password);
        }
        private static void CreateSQLDatabase(string _connect, string sqlitePath, DatabaseSchema schema, string password,
            bool createViews)
        {
            using (StreamWriter stream_writer = new StreamWriter(sqlitePath, false, System.Text.Encoding.UTF8))
            {
                foreach (TableSchema dt in schema.Tables)
                {
                    if(dt.TableName.ToLower() != "sysdiagrams")
                    {
                        try
                        {
                            string stmt = BuildCreateTableQuery(dt).Replace("AUTOINCREMENT", "IDENTITY(1,1)");
                            stmt = stmt.Replace("\n", "");
                            stmt = stmt.Replace("\t", "");
                            stmt = stmt.Replace(" COLLATE NOCASE", "");
                            stmt = stmt.Replace("DOUBLE", "decimal");
                            stream_writer.WriteLine(stmt);
                            string _strAdd = SQLGenerateStructure(dt.TableName);
                            if (!string.IsNullOrEmpty(_strAdd))
                                stream_writer.WriteLine(_strAdd);
                        }
                        catch (Exception ex)
                        {
                            _log.Error("AddSQLTable failed !", ex);
                            throw;
                        }
                    }
                }
        ...
        private static string getIndexColumns(string _connect, string _tblName)
        {
            ...
            using (SqlConnection conn = new SqlConnection(_connect))
            {
                ...
                var selectedRows = from info in allIndexColumnsSchemaTable.AsEnumerable()
                                   select new
                                   {
                ...
                {
                    if(row.TableName.ToString().ToLower()== _tblName.ToLower())
                    {
                        _rs += row.ColumnName.ToString() + ",";
                    }
                }
                conn.Close();
            }
        ...
        private static string SQLGenerateStructure(string _tblName)
        {
            string _rs = string.Empty;
            DataTable _dt = Model.Setting.TableModel.getAllDataTable(_tblName);
            if (_dt.Rows.Count > 0)
            {
                foreach (DataRow _dr in _dt.Rows)
                {
                    _rs += "insert into " + _tblName + "(" + getListColumnName(_tblName) + ") values (";
                    DataTable _dt_ = Model.Setting.TableModel.getColumnsByTable(_tblName);
                    foreach (DataRow _dr_ in _dt_.Rows)
                    {
                        ...
                        {
                            if (_dr_["type"].ToString().ToLower() == "text" || _dr_["type"].ToString().ToLower() == "ntext" || _dr_["type"].ToString().ToLower() == "varchar" || _dr_["type"].ToString().ToLower() == "nvarchar" || _dr_["type"].ToString().ToLower() == "char" || _dr_["type"].ToString().ToLower() == "vchar" || _dr_["type"].ToString().ToLower() == "xml")
                            {
        ...
        private static string getListColumnName(string _tblName)
        {
            string _rs = string.Empty;
        ...
        /*SQL 2 SQL*/
        public static void nvtam_bk(string _conntect)
        {
            using (SqlConnection conn = new SqlConnection(_conntect))
            {
                conn.Open();
